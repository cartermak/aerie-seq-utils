...
import { SeqnParser } from './language/seqn';
 ...
 */
export function seqnToSeqJson(text: string, sequenceName: string): SeqJson {
  const node = SeqnParser.parse(text);
...
function parseRequest(requestNode: SyntaxNode, text: string): Request {
  let ground_epoch: GroundEpoch | undefined;
